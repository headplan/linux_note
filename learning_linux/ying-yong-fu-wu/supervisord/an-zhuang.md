...
参考 : [http://www.cnblogs.com/kevingrace/p/7525200.html](http://www.cnblogs.com/kevingrace/p/7525200.html)
...
```
[program:memcache]
command = /usr/local/memcached/bin/memcached -p 11211 -u memcached -m 473 -c 1024 -l 127.0.0.1
autostart = true                ; 是否跟随supervisord程序启动该监控程序
startsecs = 10                  ; 启动10秒后没有异常退出,就当作已经正常启动了
autorestart = true              ; 程序异常退出后自动重启
stopasgroup = true              ; 有时候用Supervisor托管的程序还会有子进程,如果只杀死主进程,子进程就可能变成孤儿进程.
